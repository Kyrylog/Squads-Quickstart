...
import { Connection, SystemProgram, TransactionMessage, Keypair, LAMPORTS_PER_SOL } from "@solana/web3.js";
...
describe("Proposal", () => {
    it("Create a transaction proposal", async () => {
        ...
        const signature2 = await multisig.rpc.proposalCreate({
            connection,
            feePayer: secondMember, // assuming members.voter is the second member who has only vote permission
            multisigPda,
            transactionIndex,
            creator: secondMember, // assuming members.voter is the second member who has only vote permission
        });
